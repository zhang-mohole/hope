...
if args.pre_train:
    print('Begin pre_training the hourglass network...')
    for epoch in range(start, args.num_iterations):  # loop over the dataset multiple times
        ...
        with tqdm(total=trainloader.__len__(), 
            ...
            for i, tr_data in enumerate(trainloader):
                ...
                pred_hms, outputs2d, outputs3d = model(inputs, freeze_hg=args.freeze_hg)
                loss = calc_loss(pred_hms, hm)
                ...
                # loss = (lambda_pre_2d)*loss2d_init + (lambda_hm)*loss_hm
                loss = loss.mean() * 1000
                train_loss += loss.data
                loss.backward()
                optimizer.step()

                # process_bar.set_postfix_str('loss=%.5f, 2d_ini=%.5f, hm=%.5f' % (loss.data, loss2d_init.data, loss_hm.data))
...
if args.train:
    ...
    for epoch in range(start, args.num_iterations):  # loop over the dataset multiple times
        ...
        with tqdm(total=trainloader.__len__(), 
            ...
            for i, tr_data in enumerate(trainloader):
                # get the inputs
                inputs, hm, labels2d, labels3d, scale_label = tr_data
                ...
                loss3d = criterion(outputs3d, labels3d)
                # loss2d_init = criterion(outputs2d_init, labels2d)
                # loss = (lambda_1)*loss2d_init + (lambda_1)*loss2d + (lambda_2)*loss3d  + (lambda_1)*loss2d_init
                loss = (lambda_hm)*loss_hm + (lambda_1)*loss2d + (lambda_2)*loss3d
                loss.backward()
                optimizer.step()
                ...
                process_bar.set_postfix_str('loss=%.5f, 2d_hm=%.5f, 2d=%.5f, 3d=%.5f'\
                                           % (loss.data, loss_hm.data, loss2d.data, loss3d.data))
                process_bar.update()
            ...
            if args.val and (epoch+1) % args.val_epoch == 0:
                ...
                for v, val_data in enumerate(valloader):
                    # get the inputs
                    inputs, hm, labels2d, labels3d, scale_label_val = val_data
                ...
                print('val error: %.5f; 2d_hm: %.5f; 2d: %.5f; 3d: %.5f; val AUC_20_50: %.5f' % \
                     (m_val_error, val_2d_hm / (v+1), val_2d / (v+1), val_3d / (v+1), auc_val))
...
if args.test:
    ...
    for i, ts_data in enumerate(testloader):
        ...
        pred_hms, outputs2d, outputs3d = model(inputs)
        # outputs2d_init, pred_hms, outputs2d, outputs3d = model(inputs)
